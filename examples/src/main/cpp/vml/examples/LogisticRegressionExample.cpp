...
int main(int argc, char **argv){
	...
	if (argc != 6){
		cout << "Usage: " << argv[0] <<" <dataset> <numExamples> <alpha> <gamma> <numIterations>" << endl;
		exit(-1);
	}
	...
	string train_dataset = "/home/centos/vineyard/data/" + dataset + "_train.dat";
	...
	LogisticRegressionModel LR2;
	LR2.setNumClasses(numClasses);
	LR2.trainCPU(train_dataset, numExamples, alpha, gamma, numIterations);
	LR2.test(test_dataset);
